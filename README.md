...
Run `release/XIV-Gear-Lab-0.12.0-alpha.2-portable.exe`. It is an unsigned, unfinished, non-commercial preview, so Windows may show an unknown-publisher warning. It does not require installation, the game client, an FFXIV account, logs, or plugins. The desktop build creates `XIV Gear Lab Data` beside the executable and keeps all custom items, workspaces, settings and downloaded catalogues there; moving the executable and that folder together moves the complete profile.
...
# Prove that the packaged portable EXE creates and uses its sibling profile:
npm run verify:portable-data
