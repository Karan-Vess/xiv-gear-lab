...
const casterTargets = [
  { job: 'SMN', targetGcd: 2.48 },
  { job: 'RDM', targetGcd: 2.49 },
  { job: 'PCT', targetGcd: 2.50 }
...
const constraintsFor = (targetGcd: number): OptimizerConstraints => ({
  ...
  maxGcd: targetGcd,
  gcdMode: 'exact',
  gcdTargetName: `${targetGcd.toFixed(2)}s M13C magical-ranged validation target`,
...
for (const { job, targetGcd } of casterTargets) {
...
const report = {
  schemaVersion: 'm13c-magical-ranged-validation@1',
  generatedAt: new Date().toISOString(),
  snapshotId: gearSnapshot.manifest.id,
  snapshotPatch: gearSnapshot.manifest.gamePatch,
  mode: deep ? 'deep' : 'routine',
  note: 'Community values are recalculated by XIV Gear Lab for an apples-to-apples magical-ranged optimiser ablation. Job-owned trace assumptions and explicit omissions are recorded on each rotation profile.',
  rows
};
const output = resolve('artifacts/m13c-magical-ranged-validation.json');
await mkdir(resolve('artifacts'), { recursive: true });
await writeFile(output, `${JSON.stringify(report, null, 2)}\n`, 'utf8');
console.log(`Wrote ${output}`);

if (failed) {
  throw new Error('One or more curated-free magical-ranged results fell outside the declared M13C comparability threshold.');
}
