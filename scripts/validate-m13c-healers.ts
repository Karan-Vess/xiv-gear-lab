...
const healerTargets = [
  { job: 'WHM', targetGcd: 2.41 },
  { job: 'SCH', targetGcd: 2.40 },
  { job: 'AST', targetGcd: 2.43 },
  { job: 'SGE', targetGcd: 2.44 }
...
const constraintsFor = (targetGcd: number): OptimizerConstraints => ({
  ...
  maxGcd: targetGcd,
  gcdMode: 'exact',
  gcdTargetName: `${targetGcd.toFixed(2)}s M13C healer validation target`,
...
for (const { job, targetGcd } of healerTargets) {
...
const report = {
  schemaVersion: 'm13c-healer-validation@1',
  generatedAt: new Date().toISOString(),
  snapshotId: gearSnapshot.manifest.id,
  snapshotPatch: gearSnapshot.manifest.gamePatch,
  mode: deep ? 'deep' : 'routine',
  note: 'Community values are recalculated by XIV Gear Lab for an apples-to-apples healer optimiser ablation. Job-owned trace assumptions and explicit omissions are recorded on each healer rotation profile.',
  rows
};
const output = resolve('artifacts/m13c-healer-validation.json');
await mkdir(resolve('artifacts'), { recursive: true });
await writeFile(output, `${JSON.stringify(report, null, 2)}\n`, 'utf8');
console.log(`Wrote ${output}`);

if (failed) {
  throw new Error('One or more curated-free healer results fell outside the declared M13C comparability threshold.');
}
