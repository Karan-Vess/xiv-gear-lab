...
const tankTargets = [
  { job: 'PLD', targetGcd: 2.50 },
  { job: 'WAR', targetGcd: 2.50 },
  { job: 'GNB', targetGcd: 2.50 }
...
const constraintsFor = (targetGcd: number): OptimizerConstraints => ({
  ...
  maxGcd: targetGcd,
  gcdMode: 'exact',
  gcdTargetName: `${targetGcd.toFixed(2)}s M13C tank validation target`,
...
for (const { job, targetGcd } of tankTargets) {
...
const report = {
  schemaVersion: 'm13c-tank-validation@1',
  generatedAt: new Date().toISOString(),
  snapshotId: gearSnapshot.manifest.id,
  snapshotPatch: gearSnapshot.manifest.gamePatch,
  mode: deep ? 'deep' : 'routine',
  note: 'Community values are recalculated by XIV Gear Lab for an apples-to-apples tank optimiser ablation. Job-owned trace assumptions and explicit omissions are recorded on each tank rotation profile.',
  rows
};
const output = resolve('artifacts/m13c-tank-validation.json');
await mkdir(resolve('artifacts'), { recursive: true });
await writeFile(output, `${JSON.stringify(report, null, 2)}\n`, 'utf8');
console.log(`Wrote ${output}`);

if (failed) {
  throw new Error('One or more curated-free tank results fell outside the declared M13C comparability threshold.');
}
