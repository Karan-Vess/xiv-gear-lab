...
const rangedTargets = [
  { job: 'BRD', targetGcd: 2.49 },
  { job: 'MCH', targetGcd: 2.50 }
...
const constraintsFor = (targetGcd: number): OptimizerConstraints => ({
  ...
  maxGcd: targetGcd,
  gcdMode: 'exact',
  gcdTargetName: `${targetGcd.toFixed(2)}s M13C physical-ranged validation target`,
...
for (const { job, targetGcd } of rangedTargets) {
...
const report = {
  schemaVersion: 'm13c-physical-ranged-validation@1',
  generatedAt: new Date().toISOString(),
  snapshotId: gearSnapshot.manifest.id,
  snapshotPatch: gearSnapshot.manifest.gamePatch,
  mode: deep ? 'deep' : 'routine',
  note: 'Community values are recalculated by XIV Gear Lab for an apples-to-apples physical-ranged optimiser ablation. Job-owned trace assumptions and explicit omissions are recorded on each rotation profile.',
  rows
};
const output = resolve('artifacts/m13c-physical-ranged-validation.json');
await mkdir(resolve('artifacts'), { recursive: true });
await writeFile(output, `${JSON.stringify(report, null, 2)}\n`, 'utf8');
console.log(`Wrote ${output}`);

if (failed) {
  throw new Error('One or more curated-free physical-ranged results fell outside the declared M13C comparability threshold.');
}
