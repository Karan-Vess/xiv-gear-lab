...
const pilotTargets = [
  { job: 'SAM', targetGcd: 2.14 },
  { job: 'DNC', targetGcd: 2.50 },
  { job: 'BLM', targetGcd: 2.41 },
  { job: 'DRK', targetGcd: 2.46 }
...
const constraintsFor = (targetGcd: number): OptimizerConstraints => ({
  ...
  maxGcd: targetGcd,
  gcdMode: 'exact',
  gcdTargetName: `${targetGcd.toFixed(2)}s M13B validation target`,
...
for (const { job, targetGcd } of pilotTargets) {
  ...
  if (!community) throw new Error(`${job} has no ${targetGcd.toFixed(2)}s community comparison set.`);
...
const report = {
  schemaVersion: 'm13b-pilot-validation@1',
  generatedAt: new Date().toISOString(),
  snapshotId: gearSnapshot.manifest.id,
  snapshotPatch: gearSnapshot.manifest.gamePatch,
  mode: deep ? 'deep' : 'routine',
  note: 'Community rotation values are recalculated by XIV Gear Lab for an apples-to-apples optimiser ablation. Independent trace assumptions are recorded separately on each rotation profile.',
  rows
};
const output = resolve('artifacts/m13b-pilot-validation.json');
await mkdir(resolve('artifacts'), { recursive: true });
await writeFile(output, `${JSON.stringify(report, null, 2)}\n`, 'utf8');
console.log(`Wrote ${output}`);

if (failed) {
  throw new Error('One or more curated-free pilot results fell outside the declared M13B comparability threshold.');
}
